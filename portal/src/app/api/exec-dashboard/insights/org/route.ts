...
export async function POST(request: NextRequest) {
  try {
    ...
    const teamSummaries = teams.map((team) => {
      ...
      const avgCompletion = Math.round(completed.reduce((s, i) => s + i.completionRate, 0) / completed.length);
      const avgVelocity = Math.round(completed.reduce((s, i) => s + i.velocity, 0) / completed.length);
      const totalBugs = completed.reduce((s, i) => s + i.bugCount, 0);
      const lastSprint = completed[completed.length - 1];
      
      return `- ${team.teamName}: Health ${team.healthScore}% (${team.healthStatus}), Avg Completion ${avgCompletion}%, Avg Velocity ${avgVelocity} SP, Total Bugs ${totalBugs}, Last Sprint Completion ${lastSprint.completionRate}%`;
    }).join('\n');
...
Be specific with team names and numbers. Focus on actionable insights for senior leadership. Keep it concise.`;
    ...
    console.error('[ExecDashboard/OrgInsights] Error:', error.message);
    return NextResponse.json(
      { error: error.message || 'Failed to generate org insights' },
      { status: 500 }
    );
  }
}
