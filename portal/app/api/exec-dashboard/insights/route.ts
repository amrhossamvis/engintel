...
export async function POST(request: NextRequest) {
  try {
    ...
    const sprintSummary = completedSprints
      .map(
        (iter) =>
          `- ${iter.iterationName}: ${iter.completedWorkItems}/${iter.totalWorkItems} items completed (${Math.round(iter.completionRate)}%), Velocity: ${iter.velocity} SP, Bugs: ${iter.bugCount} (${iter.activeBugs} active, ${iter.newBugs} new, ${iter.resolvedBugs} resolved)`,
      )
      .join("\n");

    // Calculate trends for the prompt
    const avgVelocity = Math.round(
      completedSprints.reduce((s, i) => s + i.velocity, 0) / completedSprints.length,
    );
    const avgCompletion = Math.round(
      completedSprints.reduce((s, i) => s + i.completionRate, 0) / completedSprints.length,
    );
...
Be specific, data-driven, and concise. Reference actual numbers from the data. Use proper markdown formatting with line breaks between sections.`;
    ...
    console.error("[ExecDashboard/Insights] Error:", message);
