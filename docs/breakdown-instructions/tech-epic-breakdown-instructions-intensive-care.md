# Intensive Care Team Technical Boundary

Apply the generic technical rules below within **Intensive Care / MVO-owned scope only**. Include MVO applications, services, adapters, delivery pipelines, and operational assets only where the parent assigns them to Intensive Care. MVA, DXL, eShop, eCare, TIL, AOM, Siebel, MEF, Chordiant, and other owners are dependencies or out of scope. Do not infer MVO architecture or technologies that the parent does not provide.
